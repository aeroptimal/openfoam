from repos import ConvertOpenFoam2SU2
...
def get_mesh():
	with open('/home/foam/OpenFOAM/-7/run/constant/polyMesh/points') as f:
		x = []
		y = []
		lines = f.readlines()
		del lines [:20]
		for line in lines:
			...
			#if abs(float(data_line[1])) < 0.4 and float(data_line[0]) > -0.1 and float(data_line[0]) < 1.1:
...
class Create:
	def __init__(self,airfoilfile,DomainHeight,WakeLength,firstLayerHeight,growthRate,MaxCellSize,BLHeight,LeadingEdgeGrading,TrailingEdgeGrading,inletGradingFactor,TrailingBlockAngle):
		self.airfoilfile,self.DomainHeight,self.WakeLength,self.firstLayerHeight,self.growthRate,self.MaxCellSize,self.BLHeight,self.LeadingEdgeGrading,self.TrailingEdgeGrading,self.inletGradingFactor,self.TrailingBlockAngle=airfoilfile,DomainHeight,WakeLength,firstLayerHeight,growthRate,MaxCellSize,BLHeight,LeadingEdgeGrading,TrailingEdgeGrading,inletGradingFactor,TrailingBlockAngle
  
	def CreateFile(self):
		ChordLength=1
		NBL               =int(np.rint(math.log(1-(self.BLHeight/self.firstLayerHeight*(1-self.growthRate)))/math.log(self.growthRate)))
		MaxLayerThickness =self.firstLayerHeight*self.growthRate**NBL
		BLGrading         =MaxLayerThickness/self.firstLayerHeight
		LFF               =(self.DomainHeight/2-self.BLHeight)
		NFF               =int(np.rint(math.log(self.MaxCellSize/MaxLayerThickness)/math.log(1-MaxLayerThickness/LFF+self.MaxCellSize/LFF)))
		farfieldGrowthRate=(self.MaxCellSize/MaxLayerThickness)**(1/NFF)
		FFGrading         =self.MaxCellSize/MaxLayerThickness
		NFFA              =int(np.rint(3.14159/2*self.DomainHeight/2/self.MaxCellSize))
		
		os.chdir('/home/foam/OpenFOAM/-7/run/')
		# Import airfoil, split top and bottom, and split into quadrants
		decoded = base64.b64decode(self.airfoilfile)
		Coords            =np.loadtxt(StringIO(decoded.decode('utf-8')),skiprows=1)	#Import airfoil coordinates
		...
		rSW=self.LeadingEdgeGrading**(1/NFFA)
		if self.LeadingEdgeGrading!=1:
			dx2SW=self.LeadingEdgeGrading*LSW*((1-rSW**NFFA)/(1-rSW))**(-1)
		else:
			dx2SW=LSW/NFFA
		...
		rNW=self.LeadingEdgeGrading**(1/NFFA)
		if self.LeadingEdgeGrading!=1:
			dx2=self.LeadingEdgeGrading*LNW*((1-rNW**NFFA)/(1-rNW))**(-1)
		else:
			dx2=LNW/NFFA
		...
		NNE=int(np.rint(math.log(self.TrailingEdgeGrading)/math.log(1-dx2*(1/LNE-self.TrailingEdgeGrading/LNE))))
		dxNET=self.TrailingEdgeGrading*dx2
		...
		NSE=int(np.rint(math.log(self.TrailingEdgeGrading)/math.log(1-dx2SW*(1/LSE-self.TrailingEdgeGrading/LSE))))
		dxSET=self.TrailingEdgeGrading*dx2SW
		
		NWake=int(np.rint(math.log(self.MaxCellSize/dxSET)/math.log(1-dxSET/self.WakeLength+self.MaxCellSize/self.WakeLength)))
		WakeGrading=self.MaxCellSize/dxSET
		...
		InletGrading=self.LeadingEdgeGrading*self.DomainHeight/self.inletGradingFactor
		rInletGrading=InletGrading**(1/NFFA)
		dxInletGrading=(3.14159*self.DomainHeight/4)*((1-rInletGrading**NFFA)/(1-rInletGrading))**(-1)
		Ltop=1+math.tan(self.TrailingBlockAngle*3.14159/180)*self.DomainHeight/2
		...
		Px[0]=0
		Py[0]=0
		Px[1]=1+self.WakeLength
		Py[1]=-self.DomainHeight/2
		Px[2]=1+math.tan(self.TrailingBlockAngle*3.14159/180)*self.DomainHeight/2 
		Py[2]=-self.DomainHeight/2
		Px[3]=0
		Py[3]=-self.DomainHeight/2
		Px[4]=-self.DomainHeight/2
		Py[4]=0
		Px[5]=0
		Py[5]=self.DomainHeight/2
		Px[6]=1+math.tan(self.TrailingBlockAngle*3.14159/180)*self.DomainHeight/2
		Py[6]=self.DomainHeight/2
		Px[7]=1+self.WakeLength
		Py[7]=self.DomainHeight/2
		Px[9]=1+self.WakeLength
		Py[9]=0-math.tan(thetawake)*self.WakeLength
		Px[8]=1+self.WakeLength
		Py[8]=Py[9]+self.BLHeight
		Px[10]=1+self.WakeLength
		Py[10]=Py[9]-self.BLHeight
		Px[12]=1
		Py[12]=0
		Px[11]=1+self.BLHeight*(yTop[1]-yTop[0])/((xTop[1]-xTop[0])**2+(yTop[1]-yTop[0])**2)**0.5
		Py[11]=0-self.BLHeight*(xTop[1]-xTop[0])/((xTop[1]-xTop[0])**2+(yTop[1]-yTop[0])**2)**0.5
		Px[13]=1+self.BLHeight*(yBottom[BottomCount-1]-yBottom[BottomCount-2])/((xBottom[BottomCount-1]-xBottom[BottomCount-2])**2+(yBottom[BottomCount-1]-yBottom[BottomCount-2])**2)**0.5
		Py[13]=0-self.BLHeight*(xBottom[BottomCount-1]-xBottom[BottomCount-2])/((xBottom[BottomCount-1]-xBottom[BottomCount-2])**2+(yBottom[BottomCount-1]-yBottom[BottomCount-2])**2)**0.5
		Px[15]=0.25
		...
		Px[14]=0.25+nx15*self.BLHeight
		Py[14]=Py[15]+ny15*self.BLHeight
		Px[16]=0.25
		...
		Px[17]=0.25-nx15*self.BLHeight
		Py[17]=Py[16]-ny15*self.BLHeight
		Px[18]=-self.BLHeight
		Py[18]=0
		...
		CPx=np.zeros(2)
		CPy=np.zeros(2)
		CPx[0]=-self.DomainHeight/2*math.cos(3.14159/4)
		CPy[0]=self.DomainHeight/2*math.sin(3.14159/4)
		CPx[1]=-self.DomainHeight/2*math.cos(3.14159/4)
		CPy[1]=-self.DomainHeight/2*math.sin(3.14159/4)
		
		
		NWCPx=np.zeros(len(NWx))
		NWCPy=np.zeros(len(NWx))
		for i in range(len(NWx)):
			NWCPx[i]=NWx[i]+np.interp(NWx[i],xTop[::-1],nxTop[::-1])*self.BLHeight
			NWCPy[i]=NWy[i]+np.interp(NWx[i],xTop[::-1],nyTop[::-1])*self.BLHeight
		LNWBL=0
		for i in range(len(NWx)-1):
		...
		for i in range(len(SWx)):
			SWCPx[i]=SWx[i]+np.interp(SWx[i],xBottom,nxBottom)*self.BLHeight
			SWCPy[i]=SWy[i]+np.interp(SWx[i],xBottom,nyBottom)*self.BLHeight
		LSWBL=0
		for i in range(len(SWx)-1):
		...
		for i in range(len(NEx)):
			NECPx[i]=NEx[i]+np.interp(NEx[i],xTop[::-1],nxTop[::-1])*self.BLHeight
			NECPy[i]=NEy[i]+np.interp(NEx[i],xTop[::-1],nyTop[::-1])*self.BLHeight
		LNEBL=0
		for i in range(len(NEx)-1):
			LNEBL+=((NECPx[i+1]-NECPx[i])**2+(NECPy[i+1]-NECPy[i])**2)**0.5
		rNWBL=self.LeadingEdgeGrading**(1/NFFA)
		if self.LeadingEdgeGrading!=1:
		...
		for i in range(len(SEx)):
			SECPx[i]=SEx[i]+np.interp(SEx[i],xBottom,nxBottom)*self.BLHeight
			SECPy[i]=SEy[i]+np.interp(SEx[i],xBottom,nyBottom)*self.BLHeight
		LSEBL=0
		for i in range(len(SEx)-1):
			LSEBL+=((SECPx[i+1]-SECPx[i])**2+(SECPy[i+1]-SECPy[i])**2)**0.5
		rSWBL=self.LeadingEdgeGrading**(1/NFFA)
		if self.LeadingEdgeGrading!=1:
		...
		LTopWake=self.WakeLength-math.tan(thetawake)*self.DomainHeight/2
		...
		with open('/home/foam/OpenFOAM/-7/run/system/blockMeshDict','w') as f:
			f.write('/*--------------------------------*- C++ -*----------------------------------*\ \n')
			f.write('  =========                 | \n')
			...
			f.write('    hex (4 3 17 18 23 22 36 37) ('+str(NFFA)+' '+str(NFF)+' 1) edgeGrading ('+str(1/(InletGrading))+' '+str(1/self.LeadingEdgeGrading)+' '+str(1/self.LeadingEdgeGrading)+' '+str(1/(InletGrading))+' '+str(1/FFGrading)+' '+str(1/Grading17_3)+' '+str(1/Grading17_3)+' '+str(1/FFGrading)+' 1 1 1 1) \n')
			f.write('    hex (4 18 14 5 23 37 33 24) ('+str(NFF)+' '+str(NFFA)+' 1) edgeGrading ('+str(1/FFGrading)+' '+str(1/Grading14_5)+' '+str(1/Grading14_5)+' '+str(1/FFGrading)+' '+str(1/(InletGrading))+' '+str(1/self.LeadingEdgeGrading)+' '+str(1/self.LeadingEdgeGrading)+' '+str(1/(InletGrading))+' 1 1 1 1) \n')
			...
			f.write('    hex (15 12 11 14 34 31 30 33) ('+str(NNE)+' '+str(NBL)+' 1) egeeGrading ('+str(self.TrailingEdgeGrading)+' '+str(NEBLEGrading)+' '+str(NEBLEGrading)+' '+str(self.TrailingEdgeGrading)+' '+str(BLGrading)+' '+str(BLGrading)+' '+str(BLGrading)+' '+str(BLGrading)+' 1 1 1 1) \n')
			f.write('    hex (0 15 14 18 19 34 33 37) ('+str(NFFA)+' '+str(NBL)+' 1) edgeGrading ('+str(self.LeadingEdgeGrading)+' '+str(1/self.LeadingEdgeGrading)+' '+str(1/self.LeadingEdgeGrading)+' '+str(self.LeadingEdgeGrading)+' '+str(BLGrading)+' '+str(BLGrading)+' '+str(BLGrading)+' '+str(BLGrading)+' 1 1 1 1) \n')
			f.write('    hex (18 17 16 0 37 36 35 19) ('+str(NFFA)+' '+str(NBL)+' 1) edgeGrading ('+str(1/self.LeadingEdgeGrading)+' '+str(self.LeadingEdgeGrading)+' '+str(self.LeadingEdgeGrading)+' '+str(1/self.LeadingEdgeGrading)+' '+str(1/BLGrading)+' '+str(1/BLGrading)+' '+str(1/BLGrading)+' '+str(1/BLGrading)+' 1 1 1 1) \n')
			f.write('    hex (17 13 12 16 36 32 31 35) ('+str(NSE)+' '+str(NBL)+' 1) edgeGrading ('+str(SEBLEGrading)+' '+str(self.TrailingEdgeGrading)+' '+str(self.TrailingEdgeGrading)+' '+str(SEBLEGrading)+' '+str(1/BLGrading)+' '+str(1/BLGrading)+' '+str(1/BLGrading)+' '+str(1/BLGrading)+' 1 1 1 1) \n')
...
def GetMeshParams():	
	with open('/home/foam/OpenFOAM/-7/run/meshData.dat','r') as f:
		lines = f.readlines()
		del lines[0:28] 
		point = int(lines[0][-9:-1])
		cells = int(lines[4][-9:-1])
		del lines[0:41]
		lines = ''.join(lines)
		f1 = "Max skewness = "
		index = lines.find(f1)
		MaxSknew = float(lines[index+len(f1):index+len(f1)+7].replace(',',''))
		f2 = "Max aspect ratio"
		index = lines.find(f2)
		MaxAspectRatio = float(lines[index+len(f2)+2:index+len(f2)+8].replace(',',''))
		f3 = "Max cell openness"
		index = lines.find(f3)
		if index != -1:
			MaxCellOpenness = float(lines[index+len(f3)+2:index+len(f1)+16].replace(',',''))
		else:
			MaxCellOpenness = "not found"
		table = {
			"point":point,
			"cells":cells,
			"MaxCellOpenness":MaxCellOpenness,
...
def MeshPreview():
	os.system("/opt/paraview-5.9/bin/pvpython /home/foam/OpenFOAM/-7/run/preview.py /home/foam/OpenFOAM/-7/run")
	#Preview will generate a close and far mesh photo which will be locate in a folder we want in order to be later displayed
	
def Convert2Su2():
	with open('/home/foam/OpenFOAM/-7/run/Convert.dat','w') as f:
		f.write( '/home/foam/OpenFOAM/-7/run/constant/polyMesh\n' + '/home/foam/OpenFOAM/-7/run' + '\n' + '/Mesh.su2\n2D\nZ\n')
	ConvertOpenFoam2SU2.execute('/home/foam/OpenFOAM/-7/run/Convert.dat')